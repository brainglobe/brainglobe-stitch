...
class ImageMosaic:
    ...
    def register_shutters(
        ...
        for channel_id in l_r_pairs.keys():
            ...
            right_tile_ind = (
                0 if curr_tiles[0].illumination_name == "Right" else 1
            )
            ...
            fixed = sitk.GetImageFromArray(
                right_tile.data_pyramid[pyramid_level]
                .compute()
                .astype(np.float32)
            )
            moving = sitk.GetImageFromArray(
                left_tile.data_pyramid[pyramid_level]
                .compute()
                .astype(np.float32)
            )
    ...
    def l_r_fuse(
        ...
        resolution = (
            ...
            * self.tiles[0].resolution_pyramid[pyramid_level]
        )

        transforms = self.register_shutters(pyramid_level, resolution)
        ...
        for channel_id, (
            ...
            print(
                ...
                f"{self.channel_names[channel_id]}"
            )
            ...
            self.tiles.remove(right_tile)
            self.tile_names.remove(right_tile.name)
...
def blend_along_axis(
    ...
    im2: da.Array,
    axis: int = 2,
    blending_center_coord: float | None = None,
    pixel_width: float | None = None,
    weight_at_pixel_width: float = 0.95,
